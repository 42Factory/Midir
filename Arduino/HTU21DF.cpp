...
float HTU21DF::readHumidity(void)
{
	...
	Wire.beginTransmission(HTDU21DF_ADDRESS);
	...
	Wire.requestFrom(HTDU21DF_ADDRESS, 3);
...
void HTU21DF::setResolution(byte resolution)
{
...
byte HTU21DF::read_user_register(void)
{
  byte userRegister;
  ...
  //Read result
  Wire.requestFrom(HTDU21DF_ADDRESS, 1);
...
//Give this function the 2 byte message (measurement) and the check_value byte from the HTU21DF
...
byte HTU21DF::check_crc(uint16_t message_from_sensor, uint8_t check_value_from_sensor)
{
